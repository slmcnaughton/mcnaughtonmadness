...
router.get("/", function (req, res) {
  ...
  TournamentGroup.find(
    ...
    function (err, allTournamentGroups) {
      if (err) {
        console.log(err);
      } else {
...
router.post("/", middleware.isLoggedIn, function (req, res) {
  Tournament.findOne({ year: new Date().getFullYear() }).exec(
    function (err, foundTournament) {
      if (err) {
        console.log(err);
        res.redirect("back");
      } else {
          ...
          [
            function (callback) {
              if (res.locals.currentUser) {
                async.forEachSeries(
                  res.locals.currentUser.tournamentGroups,
                  function (tournamentGroup, next) {
...
router.get("/:groupName/bracket", function (req, res) {
  ...
  TournamentGroup.findOne({ groupName: groupName })
    ...
    .populate({
      path: "bonusAggregates",
      populate: { path: "team.id", populate: "seed" },
    })
    .populate({ path: "userTournaments", populate: "user" })
    .exec(function (err, foundTournamentGroup) {
      if (err || !foundTournamentGroup) {
